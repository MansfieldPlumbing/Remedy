...
int main(int argc, char** argv) {
    ...
    REMEDY_RECEIPT_CHECK(worker_port_start(&unsupported, &unsupported_worker) == REMEDY_ERR_NOT_SUPPORTED);
    REMEDY_RECEIPT_CHECK(unsupported_worker == REMEDY_INVALID_WORKER_TOKEN);
    ...
    REMEDY_RECEIPT_CHECK(remedy_worker_lifecycle_start(&config, &lifecycle) == REMEDY_OK);
    REMEDY_RECEIPT_CHECK(lifecycle.worker != REMEDY_INVALID_WORKER_TOKEN);
    REMEDY_RECEIPT_CHECK(lifecycle.channel != REMEDY_INVALID_CHANNEL_TOKEN);
    REMEDY_RECEIPT_CHECK(lifecycle.generation == lifecycle.worker);
    REMEDY_RECEIPT_CHECK(lifecycle.generation != 0);
    REMEDY_RECEIPT_CHECK(lifecycle.ready_received);
    REMEDY_RECEIPT_CHECK(lifecycle.state == REMEDY_WORKER_LIFECYCLE_READY);
    ...
    REMEDY_RECEIPT_CHECK(remedy_worker_lifecycle_validate_completion(
        generation,
        correlation,
    ...
    REMEDY_RECEIPT_CHECK(remedy_worker_lifecycle_request(
        &lifecycle,
        correlation,
        request,
        sizeof(request),
    ...
    REMEDY_RECEIPT_CHECK(completion_length == sizeof(request));
    for (size_t i = 0; i < sizeof(request); ++i) {
        REMEDY_RECEIPT_CHECK(completion[i] == static_cast<uint8_t>(request[i] ^ 0x5aU));
    }
    REMEDY_RECEIPT_CHECK(lifecycle.completion_received);
    REMEDY_RECEIPT_CHECK(lifecycle.correlation == correlation);
    REMEDY_RECEIPT_CHECK(lifecycle.state == REMEDY_WORKER_LIFECYCLE_COMPLETED);
    ...
    REMEDY_RECEIPT_CHECK(completion_length == 0);

    REMEDY_RECEIPT_CHECK(remedy_worker_lifecycle_quiesce_and_retire(&lifecycle) == REMEDY_OK);
    REMEDY_RECEIPT_CHECK(lifecycle.quiesce_acknowledged);
    REMEDY_RECEIPT_CHECK(lifecycle.worker_exited);
    REMEDY_RECEIPT_CHECK(lifecycle.terminal_channel_observed);
    REMEDY_RECEIPT_CHECK(lifecycle.state == REMEDY_WORKER_LIFECYCLE_RETIRED);
    REMEDY_RECEIPT_CHECK(lifecycle.worker == REMEDY_INVALID_WORKER_TOKEN);
    REMEDY_RECEIPT_CHECK(lifecycle.channel == REMEDY_INVALID_CHANNEL_TOKEN);

    bool died = false;
    REMEDY_RECEIPT_CHECK(worker_port_wait_for_death(stale_worker, 1, &died) == REMEDY_ERR_INVALID_ARGUMENT);
    REMEDY_RECEIPT_CHECK(!died);
    REMEDY_RECEIPT_CHECK(worker_port_destroy(stale_worker) == REMEDY_ERR_INVALID_ARGUMENT);
    REMEDY_RECEIPT_CHECK(channel_port_connect(stale_channel, 1) == REMEDY_ERR_INVALID_ARGUMENT);
    REMEDY_RECEIPT_CHECK(channel_port_close(stale_channel) == REMEDY_ERR_INVALID_ARGUMENT);
